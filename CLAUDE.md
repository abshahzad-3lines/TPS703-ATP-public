...
### Current Status
- **Phase**: Phase 10 complete — ATP authoring with state machine, peer review, signed export bundles, DOCX/PDF import, golden simulation, revision diff, and AI assists via Grok (xAI).
- **Main branch**: Phase 1-10 complete. v1 `test_procedures`/`test_steps` remain authoritative for live test runs; v2 `atp_definitions`/`atp_steps` (with 23 migrated rows seeded at boot) drive the new authoring UI. AI features require `XAI_API_KEY`; without it all four AI endpoints return 503 with a clear message but the rest of the system keeps working.
...
| 2026-05-17 | AI features (extract-steps, safety-warning, ordering-review, impact-summary) call xAI/Grok via OpenAI-wire-compatible `chat/completions` with `response_format: json_object` (for the three structured outputs) or plain prose (impact summary). Without `XAI_API_KEY` every AI endpoint returns 503 with a clear message; nothing else breaks. | Grok was the user's chosen provider. The OpenAI wire format means a single tiny HTTP wrapper covers both JSON and prose modes; no SDK dependency. Failing soft (503 with explanation) means the AI layer is genuinely opt-in — the rest of Phase 10 (authoring, peer review, diff, sim, import, export) all work with the key absent. |
